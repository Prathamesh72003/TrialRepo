...
public class ResponseWindowTransformerSupplier implements ValueTransformerWithKeySupplier<String, String, String> {
    ...
    public ValueTransformerWithKey<String, String, String> get() {
        return new ValueTransformerWithKey<>() {
            ...
            public void init(ProcessorContext context) {
                ...
                // Schedule cleanup based on wall clock
                            ...
                            try (KeyValueIterator<String, String> iter = windowStore.all()) {
                                ...
                                while (iter.hasNext()) {
                                    ...
                                    if (now >= windowEnd && closedStore.get(txnId) == null) {
                                        ...
                                        System.out.printf(
                                                "[TIMEOUT] %s Txn %s expired -> finalizing with defaults%n",
            ...
            public String transform(String txnId, String value) {
                ...
                try {
                    ...
                    String payload = payloadNode.isTextual()
                            ? payloadNode.asText()
            ...
            /**
             * Resolve canonical window start:
             * 1) Try DB start_time for this transaction
             * 2) Fallback to this event's timestamp
             */
            private long resolveWindowStart(String txnId, long eventTs) {
                Long txStart = finalizer.getTransactionStartEpochMillis(txnId);
                if (txStart != null) {
                    return txStart;
                }
                return eventTs;
            ...
            private String handleExternalEvent(String txnId, String payload) {
                ...
                if (wrapper == null) {
                    long start = resolveWindowStart(txnId, eventTs);
                    System.out.printf(
                            "[INFO] No active window for %s, creating new one with start=%s%n",
                            txnId,
                            new Date(start)
                    );
                    wrapper = new WindowWrapper(start);
                }
                ...
                try {
                    ...
                    System.out.printf(
                            "[UPDATE] Response stored for txn %s (total=%d)%n",
                            txnId,
                            wrapper.responses.size()
                    );
                    ...
                    System.err.printf(
                            "[ERROR] Could not parse or handle external response for %s: %s%n",
                            txnId,
                            e.getMessage()
                    );
                }
    ...
    private static class WindowWrapper {
        ...
        public WindowWrapper(long start) {
